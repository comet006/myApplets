...
Page({
  ...
  //点击切换导航的回调
  changeNav(event){
    let navId = event.currentTarget.id;//通过id向event传参的时候number会自动转化成string
    //let navId = event.currentTarget.dataset.id;
    this.setData({
      navId: navId*1, //将navId转换为number数据
      //navId: navId>>>0    //右移0位，原数值不变。非number数据会强制转换为number数据
      videoList:[]  //将上一个页面的数据清空
    })
  ...
  handlePlay(event){
    ...
    * 关键：
    * 1、如何找到上一个视频的实例对象
    *   (用this.videoContext。第一个点进来时this没有值，第二个点击事件发生时，this指向上一个视频
    ...
    * */
    ...
    //关闭上一个播放的视频
    //this.vid !== vid && this.videoContext && this.videoContext.stop();//&&表达式，当前面有值为true时才执行后面的
    //this.vid = vid;
  ...
  handleTimeUpdate(event){
    ...
    let {videoUpdateTime} = this.data;
    /*
    * 思路：判断记录播放时长的数组是否有当前视频的播放记录
    * 1、如果有，在原有的播放记录中修改为当前的播放时间
  ...
  handleEnded(event){
    //移除记录播放时长数组中当前视频的对象
    let {videoUpdateTime} = this.data;
    //videoUpdateTime.findIndex(item => item.vid === event.currentTarget.id)//找到当前元素的下标
  ...
  handleToLower(){
    ...
    videoList.push(...newVideoList);//三点运算符拆包
    this.setData({
      videoList
    })
  },
